...
This is a sample template for telegram-bot - Below is a brief explanation of what we have generated for you:

```bash
.
├── README.md                   <-- This instructions file
├── event.json                  <-- API Gateway Proxy Integration event payload
├── telegram_bot                <-- Source code for a lambda function
│   ├── utils                   <-- helpers module
│   │   ├── __init__.py
│   │   ├── helpers.py         <-- helpers for telegram cmds
│   ├── __init__.py
...
```bash
sam local invoke TelegramBotFunction --event event.json
```
...
aws cloudformation describe-stacks \
    --stack-name telegram-bot \
    --query 'Stacks[].Outputs[?OutputKey==`TelegramBotApi`]' \
    --output table
``` 
...
```bash
sam logs -n TelegramBotFunction --stack-name telegram-bot --tail
```
...
```bash
aws cloudformation delete-stack --stack-name telegram-bot
```
...
* Invoke with ``sam local invoke TelegramBotFunction --event event.json``
* Update tests
